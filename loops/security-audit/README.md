...
依存関係・lockfile・outdated を厚く見たい場合は [`deps-audit`](../deps-audit/) を使ってください。
...
#    例: ./setup/sync-ecc-assets.sh --loop security-audit --loop yabaiyo
...
- `plan.md`      : 監査計画と進捗（観点チェックリスト。ホストが `seed_files` で初回シード）
- `findings.md`  : 発見した脆弱性・不備（重大度・該当箇所・修正方針。同上）
